...
const sortFunctions = {
  ...
  name: function (a, b) { return a.name > b.name }
}
...
module.exports.getDataTypes = function getDataTypes (options) {
  let dataTypesnArray = JSON.parse(JSON.stringify(dataTypes))
  if (options) {
    if (options.category) {
      dataTypesnArray = dataTypesnArray.filter(
        (dataItem) => {
          return dataItem.category === options.category
        }
      )
    }
    dataTypesnArray = _searchArray(dataTypesnArray, options.filterString)
    dataTypesnArray = _filterDataTypesByDomain(dataTypesnArray, options.domainRestriction)
    dataTypesnArray = _sortArray(dataTypesnArray, options.sort || 'seq')
  } else {
    dataTypesnArray = _sortArray(dataTypesnArray, 'seq')
  }
  return dataTypesnArray
}
